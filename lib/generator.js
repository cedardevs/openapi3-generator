 ...
 * @param  {String} options.dest_filename Name of output file.
 * @return {Promise}
 */
const generateFile = options => new Promise((resolve, reject) => {
  ...
  const dest_filename = options.dest_filename;
  ...
  fs.readFile(path.resolve(root, file_name), 'utf8', (err, content) => {
    ...
    try {
      ...
      const generated_path = path.resolve(target_dir, dest_filename || template_path);
 ...
 * @return {Promise}
 */
codegen.generateRecurseFiles = config => new Promise((resolve, reject) => {
  bundle(config.openapi, config.base_dir)
    .catch(reject)
    .then((openapi) => {
      const openapi_dir = config.openapi
      const walker = xfs.walk(openapi_dir, {
        followLinks: false
      });

      walker.on('file', async (root, stats, next) => {
        const relative_basedir = path.relative(openapi_dir, root)
        const dest_filename = `${stats.name.split(/\./)[0]}.md` // TODO hardcoded extension

        codegen.generate({
          openapi: path.resolve(root, stats.name),
          base_dir: path.resolve(config.base_dir, relative_basedir),
          target_dir: path.resolve(config.target_dir, relative_basedir),
          dest_filename: dest_filename,
          templates: config.templates,
          curl: config.curl,
          template: config.template,
        }).then(() => {
          console.log("Done with ", stats.name)
          ...
          console.error("ERROR!!!", err.stack || err.message || inspect(err, { depth: null }))
        });
