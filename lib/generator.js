...
codegen.generate = config => new Promise((resolve, reject) => {
  bundle(config.openapi, config.base_dir)
    .catch(reject)
    .then((openapi) => {
      const orig_filename = config.openapi
      const file_path = path.relative(config.base_dir, orig_filename);
      openapi = beautifier(openapi, config);
      config.dest_filename = `${file_path.split(/\./)[0]}.md` // TODO hardcoded extension
