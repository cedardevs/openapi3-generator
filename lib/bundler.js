...
function resolveAsExternalDocs(baseDir) {
  return {
    order: 1,
    ...
    read: function readFile (file) {
      return new Promise(function (resolve, reject) {
        let content = getFileContent(file.url).then((data) => {
          let json = parseContent(data)
          let ref = file.url.replace(fs.realpathSync('.')+'/', '').replace(baseDir+'/', '').replace('.json', '.md')
          json.externalDocs = {url:ref, description: json.title}
          return json
        })
        resolve(content)
      })
...
async function dereference (json, baseDir) {
  return RefParser.dereference(`${baseDir}/`, json, {
    resolve: { zeb: resolveAsExternalDocs(baseDir), file: false },
    dereference: {
      circular: 'ignore'
    }
  })
